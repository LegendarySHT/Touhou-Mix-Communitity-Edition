...
class Preset:
	...
	var number:int = 0
	## 楽器データ一覧
	var instruments:Array = []	# Array[Array[Instrument]]
	## 解析処理用一時データ
	var bags:Array[TempSoundFontBag] = []
...
class TempSoundFontInstrument:
	var name:String = ""
	var bags:Array[TempSoundFontInstrumentBag] = []
...
## @param	base_sample		波形データ
## @param	base_key		基準のキー番号
func set_preset_sample( program_number:int, base_sample:AudioStreamWAV, base_key:int ) -> void:
...
func read_soundfont( sf:SoundFont.SoundFontData, need_program_numbers:Array = [] ) -> void:
	...
	if self.head_silent.size( ) == 0:
		var temp:Array = []
		for i in self.head_silent_samples * 2:
			temp.append( 0 )
	...
	var bag_index:int = 0
	var gen_index:int = 0
	for phdr_index in ( sf.pdta.phdr.size( ) - 1 ):
...
func _read_soundfont_pdta_inst( sf:SoundFont.SoundFontData ) -> Array:
	...
	for inst_index in ( insts.size( ) - 1 ):
			...
			for gen_count in Vector2i( gen_index, gen_next ):
				...
				match gen.gen_oper:
					...
					SoundFont.gen_oper_sustain_vol_env:
						...
						bag.adsr.sustain_vol_env_db = -clampf( float( gen.amount ), 0.0, 1440.0 ) / 10.0
					...
					SoundFont.gen_oper_initial_attenuation:
						bag.volume_db = -clampf( float( gen.amount ), 0.0, 1440.0 ) / 10.0
					#_:
					#	print( "unk", gen.gen_oper )
			# global zoneでない場合
			if bag.sample != null:
...
func _read_soundfont_preset_compose_sample( sf:SoundFont.SoundFontData, preset:Preset ) -> void:
	#
	# !Warning!
	# この関数は「長大関数」化しているが、GDScriptのメンバ関数呼び出しが遅いためすべてインライン展開したのが原因。
	# いずれ、Godot EngineのVMが **高速化したら** 関数を分割する予定。
	#
	...
	for pbag_index in preset.bags.size( ):
		...
		for ibag_index in pbag.instrument.bags.size( ):
			...
			for key_number in Vector2i( key_range.low, key_range.high + 1 ):
				...
				instrument.array_base_pitch = array_base_pitch.duplicate( )
				if ibag.original_key != 255:
					var shift_pitch:float = float( key_number - ibag.original_key ) / 12.0
					for k in instrument.array_base_pitch.size( ):
