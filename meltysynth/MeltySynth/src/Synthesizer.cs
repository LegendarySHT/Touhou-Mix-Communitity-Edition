...
namespace MeltySynth
{
    ...
    public sealed class Synthesizer : IAudioRenderer
    {
        ...
        public Synthesizer(SoundFont soundFont, SynthesizerSettings settings)
        {
            ...
            // Initialize physical channels 0-15 for backward compatibility
            for (var i = 0; i < channelCount; i++)
            {
                channels[i] = new Channel(this, i, i == percussionChannel);
            }
        ...
        /// <summary>
        /// Gets or creates a virtual channel.
        /// </summary>
        /// <param name="virtualChannelId">The virtual channel ID.</param>
        /// <returns>The channel instance.</returns>
        private Channel GetOrCreateChannel(int virtualChannelId)
        {
            if (!channels.TryGetValue(virtualChannelId, out var channel))
            {
                // Determine if this is a percussion channel based on physical channel number
                var physicalChannel = virtualChannelId % 16;
                var isPercussion = physicalChannel == percussionChannel;
                channel = new Channel(this, virtualChannelId, isPercussion);
                channels[virtualChannelId] = channel;
            }
            return channel;
        }

        /// <summary>
        /// Converts track index and physical channel to virtual channel ID.
        /// </summary>
        /// <param name="trackIndex">The MIDI track index.</param>
        /// <param name="physicalChannel">The physical MIDI channel (0-15).</param>
        /// <returns>The virtual channel ID.</returns>
        public int GetVirtualChannelId(int trackIndex, int physicalChannel)
        {
            return trackIndex * 16 + physicalChannel;
        }

        /// <summary>
        /// Parses virtual channel ID back to track index and physical channel.
        /// </summary>
        /// <param name="virtualId">The virtual channel ID.</param>
        /// <returns>A tuple of (trackIndex, physicalChannel).</returns>
        public (int trackIndex, int physicalChannel) ParseVirtualChannelId(int virtualId)
        {
            return (virtualId / 16, virtualId % 16);
        }

        /// <summary>
        /// Checks if a virtual channel exists.
        /// </summary>
        /// <param name="virtualId">The virtual channel ID to check.</param>
        /// <returns>True if the channel exists.</returns>
        public bool HasVirtualChannel(int virtualId)
        {
            return channels.ContainsKey(virtualId);
        }

        /// <summary>
        /// Gets the current number of virtual channels.
        /// </summary>
        public int MaxVirtualChannels => channels.Count;

        /// <summary>
        /// Processes a MIDI message.
        /// </summary>
        /// <param name="channel">The virtual channel to which the message is sent.</param>
        ...
        {
            var channelInfo = GetOrCreateChannel(channel);
        ...
        /// Stops a note.
        /// </summary>
        /// <param name="channel">The virtual channel of the note.</param>
        ...
        /// <summary>
        /// Gets a channel by virtual channel ID. Creates it if it doesn't exist.
        /// </summary>
        internal Channel GetChannel(int virtualChannelId) => GetOrCreateChannel(virtualChannelId);
    }
}
