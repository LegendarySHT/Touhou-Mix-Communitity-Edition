    ...
    public sealed class MidiFile
    {
        ...
        private void Load(Stream stream, int loopPoint, MidiFileLoopType loopType)
        {
            using (var reader = new BinaryReader(stream, Encoding.ASCII, true))
            {
                ...
                {
                    (messageLists[i], tickLists[i]) = ReadTrack(reader, loopType, (short)i);
                }
                ...
                if (loopPoint != 0)
                {
                    ...
                    if (loopPoint <= tickList.Last())
                    {
                        for (var i = 0; i < tickList.Count; i++)
                        {
                            if (tickList[i] >= loopPoint)
                            {
                                ...
                                messageList.Insert(i, Message.LoopStart(0));
                                break;
                            }
                        }
                    }
                    else
                    {
                        ...
                        messageList.Add(Message.LoopStart(0));
                    }
                }
        ...
        private static (List<Message>, List<int>) ReadTrack(BinaryReader reader, MidiFileLoopType loopType, short trackIndex)
        {
            var chunkType = reader.ReadFourCC();
            if (chunkType != "MTrk")
            {
            ...
            while (true)
            {
                ...
                if ((first & 128) == 0)
                {
                    ...
                    {
                        messages.Add(Message.Common(lastStatus, first, trackIndex));
                        ticks.Add(tick);
                    }
                    else
                    {
                        var data2 = reader.ReadByte();
                        messages.Add(Message.Common(lastStatus, first, data2, loopType, trackIndex));
                        ticks.Add(tick);
                    }
                ...
                switch (first)
                {
                    ...
                    case 0xFF: // Meta Event
                        switch (reader.ReadByte())
                        {
                            case 0x2F: // End of Track
                                reader.ReadByte();
                                messages.Add(Message.EndOfTrack(trackIndex));
                                ticks.Add(tick);
                            ...
                            case 0x51: // Tempo
                                messages.Add(Message.TempoChange(ReadTempo(reader), trackIndex));
                                ticks.Add(tick);
                                break;
                    ...
                    default:
                        ...
                        if (command == 0xC0 || command == 0xD0)
                        {
                            ...
                            messages.Add(Message.Common(first, data1, trackIndex));
                            ticks.Add(tick);
                        }
                        else
                        {
                            var data1 = reader.ReadByte();
                            var data2 = reader.ReadByte();
                            messages.Add(Message.Common(first, data1, data2, loopType, trackIndex));
                            ticks.Add(tick);
                        }
                        break;
                }
        ...
        internal struct Message
        {
            ...
            private Message(byte channel, byte command, byte data1, byte data2, short trackIndex = 0)
            {
            ...
            public static Message Common(byte status, byte data1, short trackIndex = 0)
            {
                byte channel = (byte)(status & 0x0F);
                byte command = (byte)(status & 0xF0);
                byte data2 = 0;
                return new Message(channel, command, data1, data2, trackIndex);
            }

            public static Message Common(byte status, byte data1, byte data2, MidiFileLoopType loopType, short trackIndex = 0)
            {
                byte channel = (byte)(status & 0x0F);
                byte command = (byte)(status & 0xF0);
            ...
            public static Message TempoChange(int tempo, short trackIndex = 0)
            {
                byte command = (byte)(tempo >> 16);
                byte data1 = (byte)(tempo >> 8);
                byte data2 = (byte)(tempo);
                return new Message((int)MessageType.TempoChange, command, data1, data2, trackIndex);
            }

            public static Message LoopStart(short trackIndex = 0)
            {
                return new Message((int)MessageType.LoopStart, 0, 0, 0, trackIndex);
            }

            public static Message LoopEnd(short trackIndex = 0)
            {
                return new Message((int)MessageType.LoopEnd, 0, 0, 0, trackIndex);
            }

            public static Message EndOfTrack(short trackIndex = 0)
            {
                return new Message((int)MessageType.EndOfTrack, 0, 0, 0, trackIndex);
            }
            ...
            public override string ToString()
            {
                switch (channel)
                {
                    ...
                    default:
                        return "T" + trackIndex + "_CH" + channel + ": " + command.ToString("X2") + ", " + data1.ToString("X2") + ", " + data2.ToString("X2");
                }
            }
            ...
            public short TrackIndex => trackIndex;
