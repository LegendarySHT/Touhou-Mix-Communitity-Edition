...
@export var item_height: float = 173  # 每个段落的高度（不含间距，因为我也不知道多少）
...
func _ready():
	...
	#从默认排序中读取
	var i =Global.default.near
	var created_album={}
	while i !=null:
		#从json数据中创建
		if created_album.has(i.album):
			i=i.p6
			continue
		...
		song.set_meta("sourceAlbumName",i.album)
		song.set_meta("songCount",Global.data[i.album].size())
		created_album[i.album]=1
